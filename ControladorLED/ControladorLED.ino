...
void loop() {
  float voltaje = analogRead(A0); // lee el valor del potenciometro
  float salida = analogRead(A1);  // lee el valor de la alimentación

  // Si el valor deseado es mayor que el valor real, aumentamos el ancho del PWM
  if(voltaje > salida) {
    pwmValue = pwmValue + 1;
    pwmValue = constrain(pwmValue, 2, 255);  // Señal modulada desde 1%-100%
    }
